...
class DietListViewModel @Inject constructor(
    ...
    private fun observeUser() {
        viewModelScope.launch {
            combine(getUserUseCase(), dietPlans) { user, plans ->
                user to plans
            }.collect { (user, plans) ->
                currentUser = user
                if (user != null) {
    ...
    private fun checkAndAutoUpdateDefaultPlan(user: User, plans: List<DietPlan>) {
        if (_isLoading.value && plans.isEmpty()) return 

        val existingDefault = plans.find { it.name.startsWith("Default") }
        if (existingDefault != null) {
            val expectedNamePrefix = "Default ${if (user.isVegetarian) "Vegetarian" else "Non-Vegetarian"}"
    ...
    fun generateDefaultDiet(replaceExistingId: Long? = null) {
        viewModelScope.launch {
            ...
            try {
                ...
                saveDietPlanUseCase(plan, meals)
            } catch (e: Exception) {
                e.printStackTrace()
